	...
	var One2manyDelete = FieldOne2Many.extend({
		...
		delete_selected_lines: function () {
			...
			if (selected_lines.length === 0) {
				return this.displayNotification({ message: _t('Veuiller selectionner au moins une ligne.'), type: 'danger' });
			}
			var w_response = confirm("Veuiller confirmer que vous voulez le supprimer.");
			if (w_response) {
